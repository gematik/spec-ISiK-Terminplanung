var BaseTreeTable = /** @class */ (function () {
    ...
    BaseTreeTable.prototype.init = function (containerId, expandPathToSelectedNode) {
        if (expandPathToSelectedNode === void 0) { expandPathToSelectedNode = false; }
        ...
        if (expandPathToSelectedNode)
            this.expandPathToSelectedNode();
    };
    BaseTreeTable.prototype.expandPathToSelectedNode = function () {
        var selected = this.getContext().querySelector(".treenode.selected");
        if (selected != undefined) {
            this.collapseAllNodes();
            this.expandPathToNode(selected.getAttribute("data-id"));
        }
    };
    BaseTreeTable.prototype.collapseAllNodes = function () {
        $(this.getContext().querySelectorAll('.vjoincollapsible, .vjoinendcollapsible')).trigger("click");
    };
    BaseTreeTable.prototype.expandPathToNode = function (id) {
        var _this = this;
        var pathToNode = this.getPathToNode(id);
        pathToNode.forEach(function (id) {
            $(_this.getContext().querySelector('[data-id="' + id + '"]').querySelectorAll('.vjoinexpandable, .vjoinendexpandable')).trigger("click");
        });
    };
    BaseTreeTable.prototype.getPathToNode = function (id) {
        var ids = [];
        var parentId = this.getContext().querySelector('[data-id="' + id + '"]').getAttribute("data-parentid");
        while (parentId != undefined) {
            ids.push(parentId);
            parentId = this.getContext().querySelector('[data-id="' + parentId + '"]').getAttribute("data-parentid");
        }
        return ids;
    };
