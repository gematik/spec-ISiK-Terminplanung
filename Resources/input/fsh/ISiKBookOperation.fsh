...
* parameter[+]
  ...
  * min = 0
  * max = "1"
  * documentation	= "Für die vorliegende Spezifikation ist die Verschiebung eines Termins ein zweistufiger Prozess, bei dem ein Termin storniert und ein neuer Termin neu gebucht wird. Dieser Parameter repräsentiert die Ressourcen-Id des stornierten Appointments. Der uri-Parameter kann eine absolute URL enthalten, Server SOLLTEN jedoch nur Termine für ihre eigene Domäne verwalten. Im neu-angelegten Appointment MUSS eine Reference auf den abgesagten Termin hinterlegt werden (vgl. Appointment.extension:replaces). Der Status der abgesagten Ressource MUSS durch den Server angepasst werden."
  * type = #uri
* parameter[+]
  * name = #schedule
  * use = #in
